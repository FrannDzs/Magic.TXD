...
*  FILE:        eirrepo/sdk/PluginHelpers.h
*  PURPOSE:     Struct plugins helper objects for quality-of-life
*
...
template <typename structType, typename factoryType, bool isDependantStruct = false>
class PluginStructRegister
{
...
public:
    inline PluginStructRegister( factoryType& theFactory, unsigned int pluginId = factoryType::ANONYMOUS_PLUGIN_ID )
    {
        this->hostFactory = nullptr;
    ...
    inline PluginStructRegister( unsigned int pluginId = factoryType::ANONYMOUS_PLUGIN_ID )
    {
        this->hostFactory = nullptr;
        this->pluginId = pluginId;
        this->structPluginOffset = factoryType::INVALID_PLUGIN_OFFSET;
    ...
    inline ~PluginStructRegister( void )
    {
    ...
    inline void RegisterPlugin( factoryType& theFactory )
    {
        if constexpr ( isDependantStruct )
        {
            this->structPluginOffset =
                theFactory.template RegisterDependantStructPlugin <structType> ( this->pluginId );
        }
        else
        {
            this->structPluginOffset =
                theFactory.template RegisterStructPlugin <structType> ( this->pluginId );
        }
    ...
    {
        return factoryType::template RESOLVE_STRUCT <structType> ( hostObj, this->structPluginOffset );
    }

    inline const structType* GetConstPluginStruct( const typename factoryType::hostType_t *hostObj )
    {
        return factoryType::template RESOLVE_STRUCT <const structType> ( hostObj, this->structPluginOffset );
    }

    inline typename factoryType::hostType_t* GetBackResolve( structType *pluginObj )
    {
        return factoryType::BACK_RESOLVE_STRUCT( pluginObj, this->structPluginOffset );
    }

    inline const typename factoryType::hostType_t* GetBackResolve( const structType *pluginObj )
    {
        return factoryType::BACK_RESOLVE_STRUCT( pluginObj, this->structPluginOffset );
    }
...
using PluginDependantStructRegister = PluginStructRegister <structType, factoryType, true>;
...
template <typename hostType, typename factoryType_meta, typename structType, typename metaInfoType>
struct factoryMetaDefault
{
    typedef factoryType_meta factoryType;
    ...
    inline void Initialize( hostType *namespaceObj )
    {
        ...
        this->endingPointPluginOffset =
            metaInfo.template RegisterPlugin <structType> ( namespaceObj, endingPointFactory_t::ANONYMOUS_PLUGIN_ID );
    }
...
template <typename hostType, typename factoryType_meta, typename structType, typename metaInfoType, typename registerConditionalType>
...
public:
    ...
    {
        return endingPointFactory_t::template RESOLVE_STRUCT <structType> ( endingPoint, metaStruct->endingPointPluginOffset );
    }

    inline structType* GetPluginStruct( hostType_t *host, endingPointType_t *endingPoint )
    {
        structType *resultStruct = nullptr;
        {
...
struct PluginDependantFactoryRegister
{
    ...
    {
        return intoFactory::template RESOLVE_PLUGIN_STRUCT <regType_t> ( obj, this->offset );
    }
...
private:
    ...
    struct factConnectionInterface : public intoFactory::pluginInterface
    {
        ...
        {
            void *structMem = pluginId.template RESOLVE_STRUCT <void> ( object, pluginOffset );
        ...
        {
            regType_t *constrObj = pluginId.template RESOLVE_STRUCT <regType_t> ( object, pluginOffset );

            if ( constrObj == nullptr )
                return;
        ...
        bool OnPluginAssign( hostedOn_t *dstObject, const hostedOn_t *srcObject, hostedPluginOffset_t pluginOffset, hostedPluginDesc_t pluginId ) override
        {
            ...
            regType_t *dstConstrObj = pluginId.template RESOLVE_STRUCT <regType_t> ( dstObject, pluginOffset );
            const regType_t *srcConstrObj = pluginId.template RESOLVE_STRUCT  <regType_t> ( srcObject, pluginOffset );
...
private:
    struct factLinkTypeInterface : public typeSysType::typeInterface
    {
        ...
        size_t GetTypeAlignment( systemPointer_t *sysPtr, void *constr_params ) const override
        {
            factoryType *factPtr = factPipelineType::getFactory( sysPtr );

            return factPtr->GetClassAlignment();
        }

        size_t GetTypeAlignmentByObject( systemPointer_t *sysPtr, const void *mem ) const override
        {
            factoryType *factPtr = factPipelineType::getFactory( sysPtr );
